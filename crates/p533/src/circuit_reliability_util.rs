use crate::path_data::{PathData, Location};
use crate::constants::*;
use crate::geometry;

/// Digital modulation signal and interferers calculation
fn digital_modulation_signal_and_interferers(path: &mut PathData, i_s: &mut [i32; MAX_MDS], i_i: &mut [i32; MAX_MDS]) -> f64 {
    /*     
...
fn equatorial_scattering(path: &mut PathData, i_s: &[i32; MAX_MDS]) {
    ...
    if path.distance <= 9000.0 && i_s[0] != NOTINDEX {
        ...
        for n in 0..MAX_MDS {
            ...
            if i_s[n] != NOTINDEX {
                let (pm, taum) = if i_s[n] < MAX_E_MDS as i32 {
                    ...
                    (path.md_e[i_s[n] as usize].ew, path.md_e[i_s[n] as usize].tau)
                } else {
                    // The mode is a F2 mode
                    (path.md_f2[i_s[n] as usize].ew, path.md_f2[i_s[n] as usize].tau)
                };

                ptspread[n] = 0.056 * pm * f64::exp(-f64::powi(tau - taum, 2) / (2.0 * f64::powi(TSPREAD, 2)));
            }
        }
        ...
        if use_cp == TRUE { // Use the control points
            for n in 0..MAX_MDS {
                // Examine all modes
                ...
                if i_s[n] != NOTINDEX && i_s[n] >= MAX_E_MDS as i32 { // Does the mode exist and is it an F2 layer mode?
                    if i_s[n] == path.n0_f2 { // Lowest order F2 mode
...
pub fn circuit_reliability(path: &mut PathData) {
    ...
    */
    ...
    path.snr = s - 10.0 * (
        ...
        10.0_f64.powf(galactic_noise / 10.0)      // Galactic noise (FaG)
    ).log10() - 10.0 * path.bw.log10() + 204.0; // kT₀ reference: -204 dBW/Hz
    ...
    path.du_sn = ((10.0_f64 * (x / y).log10()).powi(2) + (du_sd as f64).powi(2) + (du_sh as f64).powi(2)).sqrt();
    ...
    path.dl_sn = ((10.0_f64 * (y / x).log10()).powi(2) + (dl_sd as f64).powi(2) + (dl_sh as f64).powi(2)).sqrt();

    // Step 11: "Basic circuit reliability for S/N >= or < S/Nr (%)"
    
    path.bcr = if path.snr >= path.snrr {
        let bcr_calc = f64::min(130.0 - 80.0 / (1.0 + (path.snr - path.snrr) / path.dl_sn), 100.0);
        bcr_calc
    } else {
        let bcr_calc = f64::max(80.0 / (1.0 + (path.snrr - path.snr) / path.du_sn) - 30.0, 0.0);
        bcr_calc
    };
    ...
    if path.modulation == DIGITAL && path.t0 != 0.0 && path.f0 != 0.0 {
        ...
        path.rsn = if path.snr >= path.snrr {
            f64::min(130.0 - 80.0 / (1.0 + (path.snr - path.snrr) / path.dl_sn), 100.0)
        } else {
            f64::max(80.0 / (1.0 + (path.snrr - path.snr) / path.du_sn) - 30.0, 0.0)
        };
    ...
    if path.modulation == DIGITAL && path.distance <= 9000.0 {
        ...
        for n in 0..MAX_MDS {
            if i_i[n] != NOTINDEX {
                if i_i[n] < MAX_E_MDS as i32 {
                    ...
                    isumu += 10.0_f64.powf((path.md_e[i_i[n] as usize].prw - path.a + DU_IH) / 10.0);
                    isuml += 10.0_f64.powf((path.md_e[i_i[n] as usize].prw - path.a - DL_IH) / 10.0);
                } else {
                    // F2 mode interference 
                    isum += 10.0_f64.powf((path.md_f2[(i_i[n] - 3) as usize].prw - path.a) / 10.0);
                    isumu += 10.0_f64.powf((path.md_f2[(i_i[n] - 3) as usize].prw - path.a + DU_IH) / 10.0);
                    isuml += 10.0_f64.powf((path.md_f2[(i_i[n] - 3) as usize].prw - path.a - DL_IH) / 10.0);
                }
            }
        }
        ...
        path.mir = if path.sir >= path.sirr {
            f64::min(130.0 - 80.0 / (1.0 + (path.sir - path.sirr) / path.dl_si), 100.0)
        } else {
            f64::max(80.0 / (1.0 + (path.sirr - path.sir) / path.du_si) - 30.0, 0.0)
        };
            ...
            for n in 0..MAX_MDS {
                if order[n] != NOTINDEX { // The mode exists
                    for m_idx in 0..MAX_MDS {
                        if order[m_idx] != NOTINDEX { // The mode exists
