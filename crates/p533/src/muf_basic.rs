...
pub fn calculate_muf_basic(path: &mut PathData) {
    ...
    let _dhmax = (PI - e_layer_screening_frequency::incidence_angle(minele, 500.0) - (PI / 2.0 + minele)) * R0 * 2.0; // The maximum hop distance at a 500km reflection height
    //*********************************************************************************
    ...
    if path.n0_f2 != NO_LOWEST_MODE {
        ...
        if path.distance <= path.dmax {
            ...
            let n0f2dmuf = calc_f2dmuf(&path.cp[MP], path.distance / (n0 as f64 + 1.0), path.dmax, calc_b(&path.cp[MP])); // Lowest-order mode basic MUF
            ...
            // Fractional distance
            geometry::great_circle_point(path.l_tx, path.l_rx, &mut path.cp[TD02], path.distance, fracd);
            let fracd = 1.0 - 1.0 / (2.0 * (n0 as f64 + 1.0)); // R - d sub 0/2 as a fraction of the total path length
            geometry::great_circle_point(path.l_tx, path.l_rx, &mut path.cp[RD02], path.distance, fracd);
            ...
            let f2dmuf = [
                calc_f2dmuf(&path.cp[TD02], path.distance / (n0 as f64 + 1.0), dmax, calc_b(&path.cp[TD02])),
                calc_f2dmuf(&path.cp[RD02], path.distance / (n0 as f64 + 1.0), dmax, calc_b(&path.cp[RD02])),
            ];
        ...
        if path.distance <= 9000.0 {
            for n in (n0 + 1)..MAX_F2_MDS {
                if path.distance <= path.dmax {
                    ...
                    path.md_f2[n].bmuf = calc_f2dmuf(&path.cp[MP], path.distance / (n as f64 + 1.0), dmax, calc_b(&path.cp[MP]));
                } else {
                    ...
                    let mn0 = [
                        calc_f2dmuf(&path.cp[TD02], path.distance / (n0 as f64 + 1.0), calc_dmax_readonly(&path.cp[TD02]), calc_b(&path.cp[TD02])),
                        calc_f2dmuf(&path.cp[RD02], path.distance / (n0 as f64 + 1.0), calc_dmax_readonly(&path.cp[RD02]), calc_b(&path.cp[RD02])),
                    ];

                    let mn = [
                        calc_f2dmuf(&path.cp[TD02], path.distance / (n as f64 + 1.0), calc_dmax_readonly(&path.cp[TD02]), calc_b(&path.cp[TD02])),
                        calc_f2dmuf(&path.cp[RD02], path.distance / (n as f64 + 1.0), calc_dmax_readonly(&path.cp[RD02]), calc_b(&path.cp[RD02])),
                    ];
        ...
        if path.n0_e != NO_LOWEST_MODE {
            ...
            for n in n0..MAX_E_MDS {
                ...
                if path.distance < 2000.0 {
                    ...
                    path.md_e[n].bmuf = f64::min(path.cp[R1K].foe, path.cp[T1K].foe) / f64::cos(i110);
                } else {
    ...
    if path.n0_e != NO_LOWEST_MODE {
        if path.n0_f2 != NO_LOWEST_MODE {
            ...
            path.bmuf = f64::max(path.md_e[path.n0_e as usize].bmuf, path.md_f2[path.n0_f2 as usize].bmuf);
        } else {
...
pub fn calc_dmax(cp: &mut ControlPt) -> f64 {
    ...
    // Determine d sub max
    let dmax = 4780.0 + (12610.0 + 2140.0 / cp.x.powi(2) - 49720.0 / cp.x.powi(4) + 688900.0 / cp.x.powi(6)) * (1.0 / b - 0.303);
...
pub fn calc_dmax_readonly(cp: &ControlPt) -> f64 {
    ...
    // Now you can find B
    let b = cp.m3kf2 - 0.124 + (cp.m3kf2.powi(2) - 4.0) * (0.0215 + 0.005 * f64::sin(7.854 / x - 1.9635));

    // Determine d sub max
    let dmax = 4780.0 + (12610.0 + 2140.0 / x.powi(2) - 49720.0 / x.powi(4) + 688900.0 / x.powi(6)) * (1.0 / b - 0.303);
...
fn calc_b_mut(cp: &mut ControlPt) -> f64 {
    ...
    // Now you can find B
    let b = cp.m3kf2 - 0.124 + (cp.m3kf2.powi(2) - 4.0) * (0.0215 + 0.005 * f64::sin(7.854 / cp.x - 1.9635));
...
fn calc_cd(d: f64, dmax: f64) -> f64 {
    ...
    let cd = 0.74 - 0.591 * z - 0.424 * z.powi(2) - 0.090 * z.powi(3) + 0.088 * z.powi(4) + 0.181 * z.powi(5) + 0.096 * z.powi(6);
...
pub fn calc_f2dmuf(cp: &ControlPt, distance: f64, dmax: f64, b: f64) -> f64 {
    ...
    let f2dmuf = (1.0 + cd / c3k * (b - 1.0)) * cp.fof2 + cp.fh[HR_300_KM] / 2.0 * (1.0 - distance / dmax);
