        ...
        if path.md_f2[i].bmuf != 0.0 {
            ...
            if path.frequency < path.md_f2[i].muf50 {
                ...
                path.md_f2[i].fprob = f64::min(
                    1.3 - 0.8
                        / (1.0
                            + (1.0 - path.frequency / path.md_f2[i].muf50)
                                / (1.0 - path.md_f2[i].deltal)),
                    1.0,
                );
            } else {
                ...
                path.md_f2[i].fprob = f64::max(
                    0.8 / (1.0
                        + (path.frequency / path.md_f2[i].muf50 - 1.0)
                            / (path.md_f2[i].deltau - 1.0))
                        - 0.3,
                    0.0,
                );
            }
        ...
        if path.md_e[i].bmuf != 0.0 {
            ...
            if path.frequency < path.md_e[i].muf50 {
                path.md_e[i].fprob = f64::min(
                    1.3 - 0.8
                        / (1.0
                            + (1.0 - path.frequency / path.md_e[i].muf50)
                                / (1.0 - path.md_e[i].deltal)),
                    1.0,
                );
            } else {
                path.md_e[i].fprob = f64::max(
                    0.8 / (1.0
                        + (path.frequency / path.md_e[i].muf50 - 1.0)
                            / (path.md_e[i].deltau - 1.0))
                        - 0.3,
                    0.0,
                );
            }
...
pub fn find_fof2_var(path: &PathData, hour: f64, lat: f64, decile: usize) -> f64 {
    ...
    let r = lat - (lat as i32) as f64; // The fractional part of the row
    let c = hour - (hour as i32) as f64; // The fractional part of the colum
    let mut lat_l = lat as i32; // lower lat
    let mut lat_u = (lat + 1.0) as i32; // upper lat
    ...
    let mut hour_l = hour as i32; // lower hour
    let mut hour_u = (hour + 1.0) as i32; // upper hour
    ...
    let ssn = if path.ssn < 50 {
        0
    } else if path.ssn >= 50 && path.ssn <= 100 {
        1
    } else {
        2
    };
