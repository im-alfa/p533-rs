...
use crate::path_data::{Antenna, ControlPt, Location, PathData};
...
pub fn median_skywave_field_strength_short(path: &mut PathData) {
    ...
    let hr_f2 = if path.distance > path.dmax {
        f64::min(
            1490.0 / path.cp[smallest_cp_fof2(path)].m3kf2 - 176.0,
            500.0,
    ...
    if path.n0_e != NO_LOWEST_MODE {
        ...
        for n in path.n0_e as usize..MAX_E_MDS {
            ...
            // higher order modes
            {
                ...
                path.ptick =
                    f64::abs(2.0 * R0 * (f64::sin(psi) / f64::cos(delta + psi))) * (n as f64 + 1.0);
                ...
                let (at, fl, lh) = if path.distance <= 2000.0 {
                    ...
                    let fl =
                        f64::abs(path.cp[MP].fh[HR_100_KM] * f64::sin(path.cp[MP].dip[HR_100_KM]));
                    ...
                    let fl = (f64::abs(
                        path.cp[MP].fh[HR_100_KM] * f64::sin(path.cp[MP].dip[HR_100_KM]),
                    ) + f64::abs(
                        path.cp[T1K].fh[HR_100_KM] * f64::sin(path.cp[T1K].dip[HR_100_KM]),
                    ) + f64::abs(
                        path.cp[R1K].fh[HR_100_KM] * f64::sin(path.cp[R1K].dip[HR_100_KM]),
                    )) / 3.0;
                ...
                let li = (n as f64 + 1.0) * (1.0 + 0.0067 * ssn) * at
                    / (f64::powf(path.frequency + fl, 2.0) * f64::cos(aoi110));
                ...
                let lm = if path.frequency <= path.md_e[n].bmuf {
                    ...
                    f64::min(
                        46.0 * f64::powf(path.frequency / path.md_e[n].bmuf - 1.0, 0.5) + 5.0,
                        58.0,
                    )
                };
                ...
                path.md_e[n].lb = 32.45
                    + 20.0 * f64::log10(path.frequency)
                    + 20.0 * f64::log10(path.ptick)
                    + li
                    + lm
                    + lg
                    + lh
                ...
                path.md_e[n].ew =
                    136.6 + pt + gt + 20.0 * f64::log10(path.frequency) - path.md_e[n].lb;
            } else {
                    ...
                    && path.md_f2[n].fs < path.frequency)
            {
                ...
                let (at, fl, lh) = if path.distance <= 2000.0 {
                    ...
                    let at = (absorption_term(&path.cp[MP], path.month, fv)
                        ...
                        + absorption_term(&path.cp[RD02], path.month, fv))
                        / 5.0;
                    ...
                    let fl = (f64::abs(
                        ...
                        path.cp[TD02].fh[HR_100_KM] * f64::sin(path.cp[TD02].dip[HR_100_KM]),
                    ) + f64::abs(
                        path.cp[RD02].fh[HR_100_KM] * f64::sin(path.cp[RD02].dip[HR_100_KM]),
                    )) / 5.0;
                    ...
                    let lh = (find_lh(&path.cp[MP], dh, mpltime, path.month)
                        ...
                        + find_lh(&path.cp[RD02], dh, mpltime, path.month))
                        / 5.0;
                ...
                let lm = if path.frequency <= path.md_f2[n].bmuf {
                    ...
                    if path.distance <= 3000.0 {
                        f64::min(
                            36.0 * f64::powf(path.frequency / path.md_f2[n].bmuf - 1.0, 0.5) + 5.0,
                            60.0,
                        )
                    } else {
                        f64::min(
                            70.0 * (path.frequency / path.md_f2[n].bmuf - 1.0) + 8.0,
                            80.0,
                        )
                    }
                };
                ...
                path.md_f2[n].ew =
                    136.6 + pt + gt + 20.0 * f64::log10(path.frequency) - path.md_f2[n].lb;
            }
        }
    }
    ...
    let mut etw = 0.0; // Total median field strength (all modes)
    let mut ete = 0.0; // E-layer median field strength
    ...
    // Set E-layer field strengths to TINY_DB if no E-layer modes were found
    if ete == 0.0 {
        path.el = TINY_DB as f64;
        path.ei = TINY_DB as f64;
    } else {
        path.el = 10.0 * f64::log10(ete);
        path.ei = path.el; // Ei typically equals El in this implementation
    }
}
...
fn absorption_factor(cp: &ControlPt, month: i32) -> f64 {
    ...
    */
    ...
    let atno: [[f64; 29]; 9] = [
        [
            323.9, 297.5, 274.5, 256.4, 244.2, 235.0, 229.5, 226.1, 226.8, 229.0, 232.5, 237.0,
            243.4, 249.9, 258.1, 267.5, 277.5, 283.3, 283.2, 273.1, 257.0, 232.1, 201.4, 171.5,
            146.0, 123.0, 103.1, 83.0, 66.6,
        ],
        [
            312.1, 285.1, 263.1, 251.8, 249.5, 250.9, 254.5, 260.3, 266.7, 272.3, 277.8, 280.3,
            283.9, 284.5, 284.4, 283.0, 278.6, 273.0, 265.7, 256.3, 244.8, 232.0, 218.1, 204.5,
            189.9, 172.3, 155.3, 135.5, 116.2,
        ],
        [
            347.7, 321.9, 302.5, 293.8, 291.4, 289.3, 292.1, 296.6, 304.3, 313.0, 321.7, 333.8,
            342.6, 349.6, 355.2, 355.6, 352.2, 341.7, 327.3, 308.4, 286.0, 265.0, 244.1, 223.8,
            202.8, 181.8, 160.8, 141.6, 123.4,
        ],
        [
            338.0, 313.2, 297.0, 290.2, 292.1, 299.4, 308.0, 320.4, 331.6, 340.7, 347.8, 353.8,
            357.0, 360.0, 359.8, 358.3, 355.8, 350.8, 344.5, 332.7, 316.4, 292.5, 266.1, 236.4,
            214.0, 193.8, 177.5, 165.0, 155.9,
        ],
        [
            328.1, 303.8, 287.7, 282.5, 284.4, 289.4, 294.8, 303.6, 312.9, 322.7, 332.3, 343.8,
            350.6, 358.7, 364.3, 365.8, 362.4, 356.0, 346.7, 333.0, 318.8, 299.7, 282.1, 260.5,
            240.5, 220.6, 203.9, 186.3, 173.0,
        ],
        [
            305.1, 288.5, 275.2, 273.7, 278.6, 288.9, 302.5, 319.3, 333.6, 346.3, 356.3, 364.7,
            371.7, 373.6, 374.2, 373.1, 370.5, 365.1, 358.5, 347.7, 335.0, 320.3, 299.1, 276.6,
            253.2, 230.7, 214.0, 196.6, 185.3,
        ],
        [
            345.4, 319.4, 298.7, 290.1, 290.0, 291.8, 296.3, 302.9, 312.1, 320.1, 327.8, 334.1,
            340.2, 343.3, 345.7, 346.5, 345.3, 341.1, 334.5, 321.7, 304.2, 286.8, 265.9, 244.8,
            224.1, 204.5, 183.6, 164.1, 145.2,
        ],
        [
            341.9, 314.8, 295.3, 277.9, 265.0, 258.2, 254.4, 255.8, 257.3, 262.9, 268.5, 279.0,
            287.5, 295.2, 299.6, 300.2, 298.9, 291.5, 279.0, 262.6, 245.7, 227.0, 203.6, 182.3,
            163.2, 147.1, 133.9, 119.9, 110.8,
        ],
        [
            318.8, 293.3, 268.3, 251.7, 240.4, 233.1, 229.4, 228.8, 230.5, 235.5, 239.7, 242.6,
            245.4, 247.5, 248.9, 249.9, 248.5, 244.4, 237.3, 225.6, 213.5, 195.2, 172.7, 151.3,
            131.1, 113.1, 100.1, 89.0, 80.0,
        ],
    ];
...
fn absorption_layer_penetration_factor(t: f64) -> f64 {
    ...
    */
    ...
    let phi = if t < 0.0 {
        ...
        let x = (t - 0.475) / 0.475;
        let mut phi =
            (((((-0.093 * x + 0.04) * x + 0.127) * x - 0.027) * x + 0.044) * x + 0.159) * x + 0.225;
