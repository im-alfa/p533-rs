...
pub fn between_7000km_and_9000km(path: &mut PathData) {
    ...
    */
    ...
    if path.distance > 7000.0 && path.distance < 9000.0 {
        ...
        bmuf[0] = crate::muf_basic::calc_f2dmuf(
            &path.cp[TD02],
            path.distance / (path.n0_f2 as f64 + 1.0),
            dmax,
            crate::muf_basic::calc_b(&path.cp[TD02]),
        );

        let dmax = f64::min(crate::muf_basic::calc_dmax_readonly(&path.cp[RD02]), 4000.0);
        bmuf[1] = crate::muf_basic::calc_f2dmuf(
            &path.cp[RD02],
            path.distance / (path.n0_f2 as f64 + 1.0),
            dmax,
            crate::muf_basic::calc_b(&path.cp[RD02]),
        );
