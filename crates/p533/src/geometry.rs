...
pub fn great_circle_distance(here: Location, there: Location) -> f64 {
    2.0 * R0 * f64::asin(f64::sqrt(
        f64::powi(f64::sin((here.lat - there.lat) / 2.0), 2) +
        f64::cos(here.lat) * f64::cos(there.lat) * f64::powi(f64::sin((here.lng - there.lng) / 2.0), 2)
    ))
}
...
pub fn great_circle_point(here: Location, there: Location, midpnt: &mut ControlPt, distance: f64, fraction: f64) {
    if distance != 0.0 {
        midpnt.distance = distance * fraction;
        let d = distance / R0;
        let a = f64::sin((1.0 - fraction) * d) / f64::sin(d);
        let b = f64::sin(fraction * d) / f64::sin(d);
        let x = a * f64::cos(here.lat) * f64::cos(here.lng) + b * f64::cos(there.lat) * f64::cos(there.lng);
        let y = a * f64::cos(here.lat) * f64::sin(here.lng) + b * f64::cos(there.lat) * f64::sin(there.lng);
...
pub fn geomagnetic_coords(here: Location, there: &mut Location) {
    ...
    there.lat = f64::asin(
        ...
        f64::cos(here.lat) * f64::cos(geo_mag_n_pole.lat) * f64::cos(here.lng - geo_mag_n_pole.lng)
    );
    there.lng = f64::asin(
