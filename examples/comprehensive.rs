...
fn main() {
    ...
    for freq in frequencies {
        let path = create_path_prediction(
            53.3, -6.2, 40.7, -74.0, 2024, 5, 12, 100, freq, 100.0, 3.0, 2.0,
        );
    ...
    for hour in (0..24).step_by(3) {
        let path = create_path_prediction(
            53.3, -6.2, 40.7, -74.0, 2024, 5, hour, 100, 14.0, 100.0, 3.0, 2.0,
        );
    ...
    for (name, tx_lat, tx_lng, rx_lat, rx_lng) in paths {
        let path = create_path_prediction(
            tx_lat, tx_lng, rx_lat, rx_lng, 2024, 5, 12, 100, 14.0, 100.0, 3.0, 2.0,
        );

        println!(
            "{:24}  | {:6.0} km | {:5.1} | {:7.1} | {:8.1}",
    ...
    for (condition, ssn) in solar_conditions {
        let path = create_path_prediction(
            53.3, -6.2, 40.7, -74.0, 2024, 5, 12, ssn, 14.0, 100.0, 3.0, 2.0,
        );
    ...
    // Get MUF information
    let (muf50, muf90, muf10, opmuf, distance) =
        get_muf_info(53.3, -6.2, 40.7, -74.0, 2024, 5, 12, 100);
    ...
    let reliability = calculate_circuit_reliability(
        53.3, -6.2, 40.7, -74.0, 2024, 5, 12, 100, 14.0, 20.0, // 14 MHz, 20 dB required SNR
        100.0, 3.0, 2.0,
    );
